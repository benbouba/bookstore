...
export default function RestService(method, data = null, dataKey) {
  ...
  assert(
    ...
    "Invalid method for REST API request"
  )
  assert(
    dataKey && typeof dataKey === "string",
    `Invalid data key supplied for ${method}`
  )
...
RestService.getCurrentUser = (username = null, password = null) =>
  ...
  if (username && password) {
    const filteredUsers = Object.values(users).filter((user) => {
      if (
        Object.values(user).indexOf(username) !== -1 &&
            Object.values(user).indexOf(password) !== -1
      ) {
        return user
      }
    })
...
RestService.seedData = (data, key) => {
  setTimeout(() => {
    if (typeof Storage !== "undefined") {
      ...
      document.cookie = `bookstore-${key}=${JSON.stringify(
        data
