...
export const getOrders = () => async (dispatch, getState) => {
  const { userID } = getState().user.currentUserData
 ...
 */
export const addBookToOrder = (bookID, title, bookCover) => async (
  dispatch,
  getState,
) => {
  const { userID } = getState().user.currentUserData
  ...
  if (!orders) {
    ...
    const openOrderKey = Object.keys(orders).filter(
      (key) => orders[key].orderStatus === "open"
    )[0]
    const openOrderBooks = openOrderKey ? orders[openOrderKey].books : null
    if (openOrderBooks) {
      // check if the book is there and update its quantity
      const currentBookOrderIndex = openOrderBooks.findIndex(
 ...
 */
export const removeBookFromCart = (orderID, bookID) => async (dispatch, getState) => {
  const { userID } = getState().user.currentUserData
  // Get all orders
  let orders = await RestService("GET", {}, `bookstore-${userID}-orders`)
  ...
  if (orders[orderID].orderStatus === "open") {
    ...
    const bookIndex = orders[orderID].books.findIndex(
      (book) => book.bookID === bookID
    )
    if (bookIndex > -1) {
 ...
 */
export const changeQuantity = (orderID, bookID, quantity) => async (
  dispatch,
  getState
) => {
  const { userID } = getState().user.currentUserData
